...
class DataLoad:
    ...
    def create_todays_gdb(self):
        if arcpy.Exists(self.utility.todays_gdb_full_path_name()) == True:
            #arcpy.AddError("gdb already exists")
            arcpy.ExecuteError()
            sys.exit("gdb already exists") #TODO - make sure this works as expected
        else:
            print "Creating gdb"
            arcpy.CreateFileGDB_management(self.config.ETL_load_base_folder, self.utility.todays_gdb_name())
    ...
    def copy_sources(self, data_dict):
        if self.utility.valid_source_values(data_dict):
            ...
            for key, value in data_dict.iteritems():
                ...
                full_output_path = os.path.join(self.utility.todays_gdb_full_path_name(), key)
                try:
                    arcpy.Copy_management(full_input_path, full_output_path)
                except Exception:
                    arcpy.FeatureClassToFeatureClass_conversion(full_input_path, self.utility.todays_gdb_full_path_name(), key)
                except:
                    arcpy.TableToTable_conversion(full_input_path, self.utility.todays_gdb_full_path_name(), key)
        else:
    ...
    def load_data_to_gdb(self):
        try:
            ...
            if arcpy.Exists(self.utility.todays_gdb_full_path_name()):
                arcpy.Delete_management(self.utility.todays_gdb_full_path_name)
